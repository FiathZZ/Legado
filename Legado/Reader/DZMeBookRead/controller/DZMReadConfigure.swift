...
/// 阅读背景颜色列表。
///
/// The Legado reader uses the same flat paper colour as the previous reader. The
/// upstream sample's final swatch was an image pattern, which makes the page look dirty on
/// high-density displays and is deliberately not exposed by this integration.
let DZM_READ_BG_COLORS:[UIColor] = [
    ReaderThemePreset.paper.uiBackgroundColor,
    UIColor.white,
    DZM_COLOR_238_224_202,
    DZM_COLOR_205_239_205,
    DZM_COLOR_206_233_241,
    // Index 5 was the upstream image-pattern setting. Preserve that persisted index while
    // migrating it to the same flat paper background instead of showing a dark theme.
    ReaderThemePreset.paper.uiBackgroundColor
]
...
class DZMReadConfigure: NSObject {
    ...
    /// 背景颜色
    var bgColor:UIColor! {
        let index = min(max(bgColorIndex.intValue, 0), DZM_READ_BG_COLORS.count - 1)
        return DZM_READ_BG_COLORS[index]
    }
    
    /// 字体颜色
    var textColor:UIColor! {
        return ReaderThemePreset.paper.uiTextColor
...
//            paragraphStyle.lineBreakMode = .byWordWrapping
    ...
    private func initData() {
        ...
        // 背景
        if (bgColorIndex == nil) || (bgColorIndex.intValue < 0) || (bgColorIndex.intValue >= DZM_READ_BG_COLORS.count) {
