...
class DZMReadController: DZMViewController,DZMReadMenuDelegate,UIPageViewControllerDelegate,UIPageViewControllerDataSource,DZMPageViewControllerDelegate,DZMCoverControllerDelegate,DZMReadContentViewDelegate,DZMReadCatalogViewDelegate,DZMReadMarkViewDelegate {
    ...
    override func viewDidLayoutSubviews() {
        ...
        // `DZMReadContentView` was sized from UIScreen at launch in the upstream project.
        // The embedded SwiftUI host can be resized after that, so keep its coordinate space in
        // sync before laying out menu chrome.
        guard contentView != nil else { return }
        let origin = contentView.frame.origin
        contentView.frame = CGRect(origin: origin, size: view.bounds.size)
        readMenu?.updateLayout()
    }

    /// The scroll reader calls this after committing its visible page into `recordModel`.
    /// Subclasses bridge it to their presentation layer without making the imported controller
    /// depend on SwiftUI.
    func scrollReadingPositionDidChange() {
    }
