...
class DZMRMSettingView: DZMRMBaseView {
    ...
    override func addSubviews() {
        ...
        lightView.frame = CGRect(x: x, y: 0, width: 0, height: h)
        
        fontSizeView = DZMRMFontSizeView(readMenu: readMenu)
        addSubview(fontSizeView)
        fontSizeView.frame = CGRect(x: x, y: lightView.frame.maxY, width: 0, height: h)
        
        effectTypeView = DZMRMEffectTypeView(readMenu: readMenu)
        addSubview(effectTypeView)
        effectTypeView.frame = CGRect(x: x, y: fontSizeView.frame.maxY, width: 0, height: h)
        
        fontTypeView = DZMRMFontTypeView(readMenu: readMenu)
        addSubview(fontTypeView)
        fontTypeView.frame = CGRect(x: x, y: effectTypeView.frame.maxY, width: 0, height: h)
        
        bgColorView = DZMRMBGColorView(readMenu: readMenu)
        addSubview(bgColorView)
        bgColorView.frame = CGRect(x: x, y: fontTypeView.frame.maxY, width: 0, height: h)
        
        spacingView = DZMRMSpacingView(readMenu: readMenu)
        addSubview(spacingView)
        spacingView.frame = CGRect(x: x, y: bgColorView.frame.maxY, width: 0, height: h)
    }
    ...
    override func layoutSubviews() {
        ...
        let x = DZM_SPACE_SA_15
        let width = max(0, bounds.width - DZM_SPACE_SA_30)
        let height = DZM_READ_MENU_SETTING_SUB_VIEW_HEIGHT
        let views = [lightView, fontSizeView, effectTypeView, fontTypeView, bgColorView, spacingView]
        for (index, view) in views.enumerated() {
            view?.frame = CGRect(x: x, y: CGFloat(index) * height, width: width, height: height)
        }
    }
