...
class DZMReadModel: NSObject,NSCoding {
    ...
    /// Legado supplies network chapters asynchronously. DZMeBookRead's sample project
    /// expected an app-specific HTTP client here, so its network branch cannot load adjacent
    /// chapters on its own.
    var loadChapterModel: ((NSNumber, @escaping (DZMReadChapterModel?) -> Void) -> Void)?
