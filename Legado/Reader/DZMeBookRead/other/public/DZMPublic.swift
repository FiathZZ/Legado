...
/// 日志输出
func DZMLog<T>(_ message: @autoclosure () -> T) {}
...
/// The imported reader originally inferred safe areas from the dimensions of 2018 iPhones.
/// Keep a live host-provided layout instead: current devices, Split View and rotation all have
/// different insets even when their screen dimensions are identical.
struct DZMReadLayoutMetrics: Equatable {
    let bounds: CGRect
    let safeAreaInsets: UIEdgeInsets

    private static var current = DZMReadLayoutMetrics(
        bounds: UIScreen.main.bounds,
        safeAreaInsets: .zero
    )

    static func update(bounds: CGRect, safeAreaInsets: UIEdgeInsets) -> Bool {
        guard bounds.width > 0, bounds.height > 0 else { return false }

        let metrics = DZMReadLayoutMetrics(bounds: bounds, safeAreaInsets: safeAreaInsets)
        guard metrics != current else { return false }
        current = metrics
        return true
    }

    static var active: DZMReadLayoutMetrics { current }

    var readableBounds: CGRect {
        // Reading text should use the full page. The safe areas belong to transient controls
        // such as the menu, not to the permanent text canvas.
        let horizontalInset = DZM_SPACE_SA_15
        let verticalInset = DZM_SPACE_SA_8
...
var DZM_READ_RECT:CGRect! {
    DZMReadLayoutMetrics.active.readableBounds
}

/// 阅读View范围
var DZM_READ_VIEW_RECT:CGRect! {
    DZMReadLayoutMetrics.active.readingViewBounds
}
