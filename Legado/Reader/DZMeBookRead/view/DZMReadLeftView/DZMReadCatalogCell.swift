...
class DZMReadCatalogCell: UITableViewCell {
    ...
    private(set) var cachedMark: UIImageView!
    ...
    private func addSubviews() {
        ...
        cachedMark = UIImageView(image: UIImage(systemName: "checkmark.circle.fill"))
        cachedMark.tintColor = .systemGreen
        cachedMark.isHidden = true
        contentView.addSubview(cachedMark)
    ...
    override func layoutSubviews() {
        ...
        let markRightInset: CGFloat = 14
        cachedMark.frame = CGRect(x: w - markRightInset - markSize, y: (h - markSize) / 2, width: markSize, height: markSize)
        chapterName.frame = CGRect(x: DZM_SPACE_SA_15, y: 0, width: max(0, w - DZM_SPACE_SA_55), height: h)
